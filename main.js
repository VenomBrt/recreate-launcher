...
const { launchGame, installForge, getLaunchStatus, getJavaStatus, isGameRunning, stopGame, prefetchForge } = require('./minecraft');
...
const { getFriends, addFriend, removeFriend, getFriendsStatus } = require('./friends');
...
app.whenReady().then(async () => {
  ...
  // Pré-carrega a janela principal em background (sem fechar a splash)
  setTimeout(() => createMainWindow(), 2500);

  // Fallback se splash:done não chegar
  setTimeout(() => {
    if (!splashFinished) finishSplashAndOpen();
...
ipcMain.handle('friends:add', (_, username) => {
  try {
    return { success: true, friends: addFriend(username) };
  } catch (err) {
    return { success: false, error: err.message };
  }
});

ipcMain.handle('friends:remove', (_, username) => {
  return { success: true, friends: removeFriend(username) };
});

ipcMain.handle('friends:status', () => getFriendsStatus());
